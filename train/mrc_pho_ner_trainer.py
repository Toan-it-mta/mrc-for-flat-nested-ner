...
from datasets.mrc_phobert_dataset import MRCNERDataset
...
from models.model_config import PhoBertQueryNerConfig
...
class PhoBertLabeling(pl.LightningModule):
    def __init__(
            self,
        ...
        bert_config = PhoBertQueryNerConfig.from_pretrained(args.bert_config_dir,
        ...
        self.model = PhoBertQueryNER.from_pretrained(args.bert_config_dir,
                                                     config=bert_config)
    ...
    def get_dataloader(self, prefix="train", limit: int = None) -> DataLoader:
        ...
        merges_file = os.path.join(self.bert_dir, "bpe.codes")
        dataset = MRCNERDataset(json_path=json_path,
                                tokenizer=PhoBertBPETokenizer(vocab_path, merges_file),
...
def main():
    ...
    parser = PhoBertLabeling.add_model_specific_args(parser)
